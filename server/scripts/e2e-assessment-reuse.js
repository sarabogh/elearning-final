...
async function cleanupCreatedCourse(courseId) {
  if (!courseId) return;
...
async function main() {
  if (!ADMIN_TOKEN || !FACULTY_TOKEN || !LEARNER_TOKEN) {
    console.error('Missing required environment variables: ADMIN_TOKEN, FACULTY_TOKEN and LEARNER_TOKEN');
  ...
  try {
    ...
    if (CLEANUP_AFTER_RUN && createdCourseId) {
      await cleanupCreatedCourse(createdCourseId);
      console.log('Cleanup complete: removed created reuse E2E course.');
    } else if (!CLEANUP_AFTER_RUN && createdCourseId) {
